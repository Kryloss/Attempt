...
function MealCard({ meal, onDelete, onUpdate, onAddFood, onRemoveFood, onMoveFood, onEditFood, onOpenAddFood, dragHandleProps, showAdvanced = false, showTotals = true }: MealCardProps) {
    ...
    // Initialize and react to desktop two-column layout setting (reuses exercise setting)
    useEffect(() => {
        try {
    ...
    const handleFoodDragStart = (e: React.DragEvent, food: Food, index: number) => {
    ...
    const handleFoodDragOver = (e: React.DragEvent, targetIndex: number) => {
        e.preventDefault()
    ...
    const handleFoodDrop = (e: React.DragEvent, targetIndex: number) => {
        ...
        try {
            ...
            // Only reorder within the same meal
            if (data.sourceMealId === meal.id && draggedIndex !== null && draggedIndex !== targetIndex) {
                const newFoods = [...(meal?.foods || [])]
                const [moved] = newFoods.splice(draggedIndex, 1)
                newFoods.splice(targetIndex, 0, moved)
                onUpdate(meal.id, { foods: newFoods })
            }
        } catch { }
        setDraggedIndex(null)
        setDragOverIndex(null)
        // Do not stop propagation so parent meal drop handler can handle cross-meal moves
    }
        ...
        <div className="bg-white dark:bg-gray-900 border border-purple-200 dark:border-gray-700 rounded-lg p-0 shadow-sm hover:shadow-md transition-all duration-200 hover:border-purple-300 dark:hover:border-gray-600 group relative neon-surface light-surface">
            <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                    ...
                    {/* Foods List */}
                    <div className={`px-2 py-2 ${twoColumnExercises ? 'grid grid-cols-1 sm:grid-cols-2 gap-1 sm:gap-2' : 'space-y-1'}`}>
                        {(meal?.foods || []).length === 0 ? (
                            ...
                            (meal?.foods || []).map((food, index) => (
                                <div
                                    key={food.id}
                                    draggable
                                    onDragStart={(e) => handleFoodDragStart(e, food, index)}
                                ...
                                >
                                    <div className="flex-1 min-w-0 relative">
                                        {twoColumnExercises && (
                                            <div className="absolute -left-0.5 -top-0.5 w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold z-10 select-none border-2 border-green-400 text-green-600 bg-transparent shadow-[0_0_10px_rgba(16,185,129,0.45)] dark:text-green-300 dark:border-green-400">
                                                {index + 1}
                                            </div>
                                        )}
                                        <div className={`text-xs font-medium text-purple-500 dark:text-purple-50 break-words text-center ${twoColumnExercises ? 'pl-5' : ''}`}>
